      ...
      const handleThanhToan = async (vehicle) => {
        ...
        try {
          ...
          if (res && res.success && res.payment) {
            ...
            // Dùng toLowerCase() để so sánh chính xác 100% không sợ chữ hoa/thường
            if (errMsg.toLowerCase().includes("thanh toán thành công trước đó")) {
                handleCheckoutVehicle(vehicle.queueId, vehicle.licensePlate);
            } else {
                if (window.showToast) window.showToast(errMsg, "error");
            }
        } finally {
                ...
                <div className="confirm-modal-footer">
                  {selectedVehicle.statusGroup === "NoShow" ? (
                    ...
                    <>
                      ...
                      {selectedVehicle.status === "Archived" || selectedVehicle.statusGroup === "Completed" || selectedVehicle.bookingStatus === "Completed" ? (
                        <button
                          className="confirm-ok-btn w-50 fw-bold border-0 text-success"
                          style={{ background: "rgba(34, 197, 94, 0.15)", cursor: "default" }}
                          disabled={true}
                        >
                          HOÀN TẤT DỊCH VỤ
                        </button>
                      ) : isWaitingCheckoutActive(selectedVehicle) || selectedVehicle.statusGroup === "WaitingCheckout" || selectedVehicle.currentStage === "WaitingCheckout" ? (
                        <button
